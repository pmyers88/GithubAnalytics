  ...
  def main(args: Array[String]): Unit = {
    ...
    taskType match {
      case "pagerank" =>
        ...
        val users = sc.textFile("/Users/phil/Development/github-analytics/usernames.txt").map { line =>
      ...
      case "sp" =>
        val sourceId: VertexId = 2//options.remove("sourceId").map(_.toLong)
        //options.foreach {
          //case (opt, _) => throw new IllegalArgumentException("Invalid option: " + opt)
        //}
        ...
        println(sssp.vertices.filter{ case (id, (dist, ids)) => !dist.isPosInfinity }.collect.mkString("\n"))
